...
// src/ssr-commerce-next.index.ts
var ssr_commerce_next_index_exports = {};
__export(ssr_commerce_next_index_exports, {
...
module.exports = __toCommonJS(ssr_commerce_next_index_exports);
...
// src/ssr-next/commerce/types/controller-constants.ts
...
// src/ssr-next/commerce/controllers/breadcrumb-manager/headless-core-breadcrumb-manager.ssr.ts
...
// src/ssr-next/commerce/controllers/cart/headless-cart.ssr.ts
function defineCart() {
  return {
    listing: true,
    search: true,
...
// src/ssr-next/commerce/controllers/context/headless-context.ssr.ts
...
// src/ssr-next/commerce/controllers/did-you-mean/headless-did-you-mean.ssr.ts
...
// src/ssr-next/commerce/controllers/filter-suggestions/headless-filter-suggestions-generator.ssr.ts
...
// src/ssr-next/commerce/controllers/generator/headless-commerce-facet-generator.ssr.ts
function defineFacetGenerator() {
  return {
    listing: true,
    search: true,
...
// src/ssr-next/commerce/controllers/instant-products/headless-instant-products.ssr.ts
...
// src/ssr-next/commerce/controllers/pagination/headless-core-commerce-pagination.ssr.ts
...
// src/ssr-next/commerce/controllers/parameter-manager/headless-core-parameter-manager.ssr.ts
...
// src/ssr-next/commerce/controllers/product-enrichment/headless-product-enrichment.ssr.ts
...
// src/ssr-next/commerce/controllers/product-list/headless-product-list.ssr.ts
function defineProductList() {
  return {
    listing: true,
    search: true,
...
// src/ssr-next/commerce/controllers/product-view/headless-product-view.ssr.ts
...
// src/ssr-next/commerce/controllers/recent-queries-list/headless-recent-queries-list.ssr.ts
...
// src/ssr-next/commerce/controllers/recommendations/headless-recommendations.ssr.ts
...
// src/ssr-next/commerce/controllers/search-box/headless-search-box.ssr.ts
...
// src/ssr-next/commerce/controllers/sort/headless-core-commerce-sort.ssr.ts
...
// src/ssr-next/commerce/controllers/standalone-search-box/headless-standalone-search-box.ssr.ts
...
// src/ssr-next/commerce/controllers/summary/headless-core-summary.ssr.ts
...
// src/ssr-next/commerce/controllers/triggers/headless-notify-trigger.ssr.ts
...
// src/ssr-next/commerce/controllers/triggers/headless-query-trigger.ssr.ts
...
// src/ssr-next/commerce/controllers/triggers/headless-redirection-trigger.ssr.ts
...
var ControllerBuilder = class {
  constructor(definition3, engine, props) {
...
// src/ssr-next/commerce/controller-utils.ts
...
function buildControllerDefinitions({
  ...
  const controllerMap = mapObject(definitionsMap, (definition3, key) => {
    ...
    return new ControllerBuilder(definition3, engine, props).withAdditionalArgs([solutionType]).build();
  });
  return filterObject(
    controllerMap,
    (value) => value !== null
...
// src/ssr-next/commerce/utils/controller-wiring.ts
var import_bueno92 = require("@coveo/bueno");

// src/ssr-next/commerce/utils/recommendation-filter.ts
function isRecommendationDefinition(controllerDefinition) {
  const hasRecommendationFlag = "recommendation" in controllerDefinition && controllerDefinition.recommendation === true;
  const hasValidSlotId = recommendationInternalOptionKey in controllerDefinition && "slotId" in controllerDefinition[recommendationInternalOptionKey];
  return hasRecommendationFlag && hasValidSlotId;
}
function getRecommendationDefinitions(controllerDefinitions) {
  const recommendationMap = {};
  for (const [name, definition3] of Object.entries(controllerDefinitions)) {
    if (isRecommendationDefinition(definition3)) {
      recommendationMap[name] = definition3;
    }
  }
...
// src/ssr-next/commerce/utils/controller-wiring.ts
var requiredDefinition = {
  context: new import_bueno92.RecordValue({
    options: { required: true },
    values: contextDefinition
  })
};
var listingDefinition = {
  ...requiredDefinition,
  searchParams: new import_bueno92.RecordValue({
    values: parametersDefinition
  })
};
var standaloneDefinition = {
  ...requiredDefinition
};
...
var recommendationsDefinition = (recommendationName) => ({
  ...requiredDefinition,
  recommendations: new import_bueno92.ArrayValue({
    each: new import_bueno92.StringValue({
      required: true,
...
var listingDefinitionSchema = new import_bueno92.Schema(listingDefinition);
var searchDefinitionSchema = new import_bueno92.Schema(searchDefinition);
var standaloneDefinitionSchema = new import_bueno92.Schema(standaloneDefinition);
var recommendationsDefinitionSchema = (recommendationName) => new import_bueno92.Schema(recommendationsDefinition(recommendationName));
function validateBuildConfig(solutionType, controllerDefinitions, buildConfig) {
  const validationMap = {
    ["listing" /* listing */]: listingDefinitionSchema,
    ["search" /* search */]: searchDefinitionSchema,
    ["standalone" /* standalone */]: standaloneDefinitionSchema,
    ["recommendation" /* recommendation */]: recommendationsDefinitionSchema(
...
function wireControllerParams(solutionType, controllerDefinitions, buildConfig) {
...
// src/ssr-next/commerce/factories/build-factory.ts
...
var buildFactory = (controllerDefinitions, options) => (solutionType) => async (buildOptions) => {
  ...
  const enabledRecommendationControllers = buildOptions && "recommendations" in buildOptions ? buildOptions?.recommendations.length : 0;
...
// src/ssr-next/commerce/validation/controller-validation.ts
var validateControllerNames = (controllers) => {
  const reservedNames = ["context", "cart", "parameterManager"];
  const invalidNames = Object.keys(controllers).filter(
    (name) => reservedNames.includes(name)
  );
...
// src/ssr-next/commerce/engine/commerce-engine.ssr.ts
...
var registerStandaloneSearchBox2 = (0, import_toolkit122.createAction)(
  ...
  (payload) => validatePayload(payload, {
    id: requiredNonEmptyString,
    redirectionUrl: requiredNonEmptyString,
    overwrite: new import_bueno93.BooleanValue({ required: false })
  })
);
...
var fetchRedirectUrl2 = (0, import_toolkit122.createAsyncThunk)(
  ...
  async (payload, {
    ...
    validatePayload2(payload, { id: new import_bueno93.StringValue({ emptyAllowed: false }) });
...
var foldingOptionsSchemaDefinition = {
  collectionField: new import_bueno94.StringValue({ emptyAllowed: false, required: false }),
  parentField: new import_bueno94.StringValue({ emptyAllowed: false, required: false }),
  childField: new import_bueno94.StringValue({ emptyAllowed: false, required: false }),
  numberOfFoldedResults: new import_bueno94.NumberValue({ min: 0, required: false })
};
...
var templateSchema = new import_bueno95.Schema({
  content: new import_bueno95.Value({ required: true }),
  conditions: new import_bueno95.Value({ required: true }),
  priority: new import_bueno95.NumberValue({ required: false, default: 0, min: 0 }),
  fields: new import_bueno95.ArrayValue({
    required: false,
    each: requiredNonEmptyString
  })
});
function buildTemplatesManager() {
  ...
  const validateTemplate = (template) => {
    ...
    if (!areConditionsValid) {
      throw new import_bueno95.SchemaValidationError(
...
function applyToUrlSearchParam(urlSearchParams, previousState, pair) {
  if ((0, import_bueno97.isNullOrUndefined)(pair[1])) {
    return;
  }
  if (isSortPair(pair)) {
...
function extendSearchParams(commerceSearchParams, key, value) {
  if ((0, import_bueno97.isNullOrUndefined)(value)) {
    return;
  }
  if (key === "sortCriteria") {
...
var getProductProperty = (product, property) => {
  ...
  if (!(0, import_bueno98.isNullOrUndefined)(anyProduct[property])) {
    return anyProduct[property];
  }
  if (!(0, import_bueno98.isNullOrUndefined)(product.additionalFields[property])) {
...
var fieldsMustBeDefined = (fieldNames) => {
  return (product) => {
    return fieldNames.every(
      (fieldName) => !(0, import_bueno98.isNullOrUndefined)(getProductProperty(product, fieldName))
    );
  };
};
...
var getFieldValuesFromProduct = (fieldName, product) => {
  ...
  return (0, import_bueno98.isArray)(rawValue) ? rawValue : [rawValue];
};
