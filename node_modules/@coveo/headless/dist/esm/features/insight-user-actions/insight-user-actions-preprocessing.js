import { UserActionType, } from './insight-user-actions-state.js';
const SECOND = 1000;
 ...
 */
export const preprocessUserActionsData = (state, actions) => {
    ...
    const timeline = splitActionsIntoTimelineSessions(mappedAndSortedActions, ticketCreationTimestamp, excludedCustomActions);
    return timeline;
};
const mapRawActionToUserAction = (rawAction) => {
    const actionData = JSON.parse(rawAction.value);
    return {
        actionType: rawAction.name,
 ...
 * @returns {UserAction[]}
 */
export const mapAndSortActionsByMostRecent = (rawUserActions) => {
    let parsingError = false;
    const parsedActions = rawUserActions
        .reduce((acc, rawAction) => {
        try {
            const userAction = mapRawActionToUserAction(rawAction);
            acc.push(userAction);
        }
        catch (_) {
            parsingError = true;
 ...
 * @returns {boolean}
 */
export const isActionWithinSessionThreshold = (action, latestTimestampInSession) => {
    return (Math.abs(action.timestamp - latestTimestampInSession) <
 ...
 * @returns {void}
 */
export const insertSessionInTimeline = (session, ticketCreationDate, timeline) => {
    if (!session.actions?.length) {
        return;
    }
    if (ticketCreationDate >= session.start - SESSION_INACTIVITY_THRESHOLD_IN_MS &&
        ticketCreationDate <= session.end + SESSION_INACTIVITY_THRESHOLD_IN_MS) {
        const ticketCreationAction = {
 ...
 * @returns {boolean}
 */
export const shouldExcludeAction = (action, excludedCustomActions) => {
    ...
    if (action.actionType === UserActionType.CUSTOM) {
        ...
        return (excludedCustomActions.includes(eventType) ||
 ...
 */
export const splitActionsIntoTimelineSessions = (actions, ticketCreationDate, actionsToExclude) => {
    ...
    actions.forEach((action) => {
        ...
        currentSession.actions = currentSession.actions.filter((action) => !shouldExcludeAction(action, actionsToExclude));
    ...
    return {
        ...
        followingSessions: returnTimeline.followingSessions.slice(returnTimeline.followingSessions.length - NUMBER_OF_FOLLOWING_SESSIONS),
    };
};
