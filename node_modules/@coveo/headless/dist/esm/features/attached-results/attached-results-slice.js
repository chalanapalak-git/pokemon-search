...
import { getAttachedResultsInitialState, } from './attached-results-state.js';
export const attachedResultsReducer = createReducer(getAttachedResultsInitialState(), (builder) => {
    builder
        ...
        if (!isNullOrUndefined(action.payload.permanentId) ||
            !isNullOrUndefined(action.payload.uriHash)) {
            state.results = [...state.results, action.payload];
        }
    })
        .addCase(detachResult, (state, action) => {
        state.results = state.results.filter((result) => attachedResultsMatchIds(result, action.payload));
    });
});
const attachedResultsMatchIds = (result1, result2) => {
    const isPermanentIdEqual = !isNullOrUndefined(result1.permanentId) &&
        result1?.permanentId === result2?.permanentId;
    const isUriHashEqual = !isNullOrUndefined(result1.uriHash) &&
