 ...
 */
export const baseCaseAssistRequest = (req, method, contentType, path, queryStringArguments = {}) => {
...
export const prepareSuggestionRequestFields = (fields) => Object.keys(fields)
    .filter((fieldName) => fields[fieldName].value !== '')
    .reduce((obj, fieldName) => {
    obj[fieldName] = fields[fieldName];
    return obj;
}, {});
export const prepareContextFromFields = (fields) => Object.keys(fields)
    .filter((fieldName) => fields[fieldName].value !== '')
    .reduce((obj, fieldName) => {
    obj[fieldName] = fields[fieldName].value;
    return obj;
}, {});
const validateCaseAssistRequestParams = (req) => {
    if (!req.url) {
    ...
    if (!req.caseAssistId) {
        throw new Error("The 'caseAssistId' attribute must contain a valid Case Assist configuration ID.");
    }
};
