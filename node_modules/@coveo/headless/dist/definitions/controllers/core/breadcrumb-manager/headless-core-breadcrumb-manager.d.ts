...
import type { CategoryFacetSection, ConfigurationSection, DateFacetSection, FacetSection, NumericFacetSection, SearchSection } from '../../../state/state-sections.js';
...
export type GetBreadcrumbsConfiguration<T extends AnyFacetSetState> = {
    engine: CoreEngine<ConfigurationSection & SearchSection & FacetSection & NumericFacetSection & DateFacetSection & CategoryFacetSection>;
    facetSet: T;
    ...
    facetValuesSelector: (state: CoreEngine<ConfigurationSection & SearchSection & FacetSection & NumericFacetSection & DateFacetSection & CategoryFacetSection>['state'], facetId: string) => InferFacetSliceValueType<T>[];
};
type InferFacetSliceValueRequestType<T extends AnyFacetSetState> = T[string]['request']['currentValues'][number];
type InferFacetSliceValueType<T extends AnyFacetSetState> = InferFacetSliceValueRequestType<T> extends FacetValueRequest ? FacetValue : InferFacetSliceValueRequestType<T> extends NumericRangeRequest ? NumericFacetValue : InferFacetSliceValueRequestType<T> extends DateRangeRequest ? DateFacetValue : CategoryFacetValue;
export declare const getBreadcrumbs: <T extends AnyFacetSetState>(config: GetBreadcrumbsConfiguration<T>) => Breadcrumb<InferFacetSliceValueType<T>>[];
/**
 ...
 */
export declare function buildCoreBreadcrumbManager(engine: CoreEngine): BreadcrumbManager;
export {};
