import type { SearchEngine } from '../../../../app/search-engine/search-engine.js';
import { type Context } from '../../../../controllers/context/headless-context.js';
import type { ContextProps } from '../../../../controllers/core/context/headless-core-context.js';
import type { ControllerDefinitionWithProps } from '../../../common/types/controllers.js';
export * from '../../../../controllers/context/headless-context.js';
export interface ContextDefinition extends ControllerDefinitionWithProps<SearchEngine, Context, ContextProps> {
}
/**
 * Defines a `Context` controller instance.
 * @group Definers
 *
 * @returns The `Context` controller definition.
 * */
export declare function defineContext(): ContextDefinition;
